...
feature "visitor runs a search from home page and sees results" do
  scenario "visits home and selects a search" do
    user1 = User.create(
      email: "user@example.com", password: "password", username: "Cool User"
    )
    user2 = User.create(
      email: "user2@example.com", password: "password2", username: "User 2"
    )
    suzi = User.create(
      email: "suzi@fakeemail.com", password: "PASSWORD", username: "Suzi Curran", admin: true
    )

    balrog = Monster.create(
      user_id: user1.id, name: 'Durin\'s Bane', source: 'http://lotr.wikia.com/wiki/Durin\'s_Bane',
    ...
    blooky = Monster.create(
      user_id: user1.id, name: 'Napstablook',
      ...
      in the Ruins as Undertale\'s first miniboss.', email: 'Napstablook22@undernet.com'
    )

    cookiemonster = Monster.create(user_id: user1.id, name: 'Cookie Monster',
      ...
      and "Om nom nom nom".', email: 'mewantcookie@sstreet.org'
    )
